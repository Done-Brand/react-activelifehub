...
// Replace this with your actual method of retrieving the current user's ID
const ProductModal = ({ product, isOpen, setIsOpen }) => {
  ...
  useEffect(() => {
    ...
    //     if (response.ok) {
    //       const userId = await response.text(); // Read the response as plain text
  ...
  useEffect(() => {
    console.log("Current user ID state:", userId); // Log the user ID state whenever it changes
  }, [userId]);
  ...
  const handleAddToCart = async (e) => {
    ...
    const cartData = {
      ...
      quantity: 1, // Default quantity, can be changed based on your logic
    };
